...
input.forEach(rule => {
	...
	let color = parts[0].split(" bags")[0];
	rules[color] = [];
	if(parts[1] !== "no other bags.") {
		let options = parts[1].substring(0, parts[1].length - 1).split(", ");
		options.forEach(option => {
			let optionCount = +option.split(" ")[0];
			let optionColor = option.substring(2, option.indexOf(" bag"));
			
			inverse[optionColor] = inverse[optionColor] || [];
			inverse[optionColor].push(color);
			console.log(inverse[optionColor]);
			
			rules[color].push({color: optionColor, count: optionCount});
		});
	}
});
...
// Part 1
let options = [];
let candidates = ["shiny gold"];
while(candidates.length) {
	let newCandidates = [];
	for(var c of candidates) {
		console.log(c);
		(inverse[c] || []).forEach(x => newCandidates.push(x));
	}
	newCandidates.forEach(x => options.push(x));
	candidates = newCandidates;
}
console.log(new Set(options).size);

// Part 2
let counts = {};
function getCounts(bagColor) {
	if(bagColor in counts) {
		return counts[bagColor];
	}
	
	let result = 1;
	rules[bagColor].forEach(rule => {
		result += getCounts(rule.color) * rule.count;
	});
	counts[bagColor] = result;
	
	return result;
}
console.log(getCounts("shiny gold") - 1);
