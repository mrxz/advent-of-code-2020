...
input.forEach(line => {
	if(line === "") {
		// Check valid
		if(required.every(x => x in currentPassword)) {
			// valid++;
			// Part 2 extended validation
			isValid = true;
			isValid &= +currentPassword['byr'] >= 1920 && +currentPassword['byr'] <= 2002;
			isValid &= +currentPassword['iyr'] >= 2010 && +currentPassword['iyr'] <= 2020;
			isValid &= +currentPassword['eyr'] >= 2020 && +currentPassword['eyr'] <= 2030;
			// Height
			height = currentPassword['hgt'];
			heightVal = +height.substr(0, height.length - 2);
			if(height.substr(-2) === "in") {
				isValid &= heightVal >= 59 && heightVal <= 76;
			} else if(height.substr(-2) === "cm") {
				isValid &= heightVal >= 150 && heightVal <= 193;
			} else {
				isValid = false;
			}
			isValid &= /^#[a-f0-9]{6}$/g.test(currentPassword['hcl']);
			isValid &= ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].indexOf(currentPassword['ecl']) !== -1;
			isValid &= /^[0-9]{9}$/g.test(currentPassword['pid']);
			if(isValid) {
				valid++;
			}
		}
	...
	parts = line.split(" ");
	parts.forEach(part => {
		currentPassword[part.split(":")[0]] = part.split(":")[1];
	});
});
