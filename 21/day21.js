...
"gptcng md hdgfgj mpm bpbdlmg lpznnc tk hchdc xrrcj tkqng tdbcfb rhcj kflk xdkhj dtdbzq zfrlbz ngrnd dfxmxd jpvfc mgngg xppdk xrtx gnsxbz csmfzp nptr qtncs vrjj bhp cztkz gpgf mjjgj bbxmx jchrn cnhch rdnhkqz vdhtgm gkcqxs lbgntt cpvn sxlj vmhf vzzgts sgn ndssl ndgfp qkdgt stnlkxj gxxg mxkh vcx qss mvv rgc smpsr sp cppzn kfpx fczmkt hvgh mckrjz sjzsc cxjkfz xbdsf bvh kbtxv cbmnb sjvrgx jdmhd jjgvd nsvfk mvfqbnlb jckpd dpcmr bbmpr dgcq (contains nuts)",
    ...
    const allergenString = parts[1].split("contains ")[1];
    const allergens = allergenString.substring(0, allergenString.length - 1).split(", ");
...
// Determine candidate ingredients for allergens
const allAllergens = new Set([].concat(...input.map(i => i.allergens)));
console.log(allAllergens);

const allergenCandidateMap = {}
for(var allergen of allAllergens) {
    console.log(allergen);
    
    const candidates = input.filter(i => i.allergens.indexOf(allergen) !== -1)
        .map(i => i.ingredients)
        .reduce((a,b) => a.filter(i => b.indexOf(i) !== -1));
    allergenCandidateMap[allergen] = candidates;
}

const allCandidateIngredients = new Set([].concat(...Object.values(allergenCandidateMap)));

// 
console.log(allCandidateIngredients);
const result = [].concat(...input.map(i => i.ingredients)).filter(i => !allCandidateIngredients.has(i));
console.log(result, result.length);

// Part 2, figure out what is what
const mapping = {}
outerLoop:
while(Object.keys(allergenCandidateMap).length > 0) {
    // Find one that is certain
    for(let allergen of Object.keys(allergenCandidateMap)) {
        console.log(allergen);
        if(allergenCandidateMap[allergen].length == 1) {
            console.log("Allergen can be known for sure :-), it's", allergenCandidateMap[allergen][0]);
            mapping[allergen] = allergenCandidateMap[allergen][0];
            delete allergenCandidateMap[allergen];
            // Remove it from the others
            for(let allergen2 of Object.keys(allergenCandidateMap)) {
                allergenCandidateMap[allergen2] = allergenCandidateMap[allergen2].filter(x => x!== mapping[allergen]);
            }
            break;
        }
    }
...
console.log(Object.keys(mapping).sort().map(x => mapping[x]).join(","));
