...
const set = (x, y, board, value) => {
    board[y] = board[y].substring(0, x) + value + board[y].substring(x + 1);
}

const countOccupied = (x,y,board) => {
    const asNum = (x,y,dx,dy) => useRays ? (ray(x,y,dx,dy,board) == '#' ? 1 : 0) : (get(x+dx,y+dy,board) == '#' ? 1 : 0);
    return asNum(x,y,-1,-1) +
        asNum(x,y,0,-1) +
        asNum(x,y,1,-1) +
        asNum(x,y,-1,0) +
        asNum(x,y,1,0) +
...
const step = (board) => {
    ...
    for(var x = 0; x < board[0].length; x++) {
        for(var y = 0; y < board[0].length; y++) {
            const tile = get(x,y,board);
            if(tile === '.') {
                continue;
            }
            
            const count = countOccupied(x,y,board);
            if(count == 0 && tile === 'L') {
                set(x, y, newBoard, '#');
            } else if(count >= tolerance && tile === '#') {
                set(x, y, newBoard, 'L');
            }
        }
    }
...
do {
    ...
    console.log(i++);
    printBoard(state);
} while(asString(state) != asString(prev))

const resultBoard = asString(state);
console.log(resultBoard.length - resultBoard.replace(/#/g, '').length);
