...
const parseRule = (line) => {
    ...
    const options = relevant.split(" | ").map(option => {
        // Literal
        if(option[0] === '"') {
            return option[1]; // Assume single character.
        } else {
            return option.split(" ").map(x => +x);
        }
    });
    return [index, options];
};

const rules = rulesRaw.map(parseRule).reduce((map, entry) => {
    map[entry[0]] = entry[1];
    return map;
}, {});
...
rules[newRules[0][0]] = newRules[0][1];
rules[newRules[1][0]] = newRules[1][1];
...
"babaabbabaabbabbbbaaabbbbbbbbabbbbbbbb",
...
const result = input.map(line => {
    // Check if a rule matches on the line starting at a certain index.
    const match = (rule, line, i) => {
        //console.log("Matching rule ", rule, i);
        const results = [];
        // At the start of the rule
        return [].concat(...rule.map(option => {
            if(typeof option === 'string') {
                // Match literal
                if(line[i] === option) {
                    // If it matches, there is only one result, consume and nothing else to do.
                    //console.log('\tLiteral match')
                    return [
                        i + 1
                    ];
                } else {
                    //console.log('\tNo literal match')
                    return [];
                }
            } else {
                // Match all rules in sequence of this option
                const result = [];
                let queue = [{ruleIndex: 0, lineIndex: i}];
                do {
                    let state = queue.pop();
                    let possiblities = match(rules[option[state.ruleIndex]], line, state.lineIndex);
                    for(var j = 0; j < possiblities.length; j++) {
                        const pos = possiblities[j];

                        if(state.ruleIndex + 1 === option.length) {
                            result.push(pos);
                        } else {
                            queue.push({ruleIndex: state.ruleIndex + 1, lineIndex: pos});
                        }
                    }
                } while(queue.length)
                return result;
    ...
    let outcome = match(rules[0], line, 0);
    return outcome.some(x => x == line.length);
}).filter(x => x);
