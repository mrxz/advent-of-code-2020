...
input.forEach(input => {
	...
	const letter = policy.split(" ")[1][0];
	const minLength = +policy.split(" ")[0].split("-")[0];
	const maxLength = +policy.split(" ")[0].split("-")[1];
	
	const length = [...password].filter(x => x == letter).length;
...
input.forEach(input => {
	...
	const letter = policy.split(" ")[1][0];
	const lowerIndex = +policy.split(" ")[0].split("-")[0] - 1;
	const higherIndex = +policy.split(" ")[0].split("-")[1] - 1;
	
	if((password[lowerIndex] == letter || password[higherIndex] == letter)
		&& password[lowerIndex] != password[higherIndex]) {
		valid++;
	}
});
