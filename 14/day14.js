...
const instructions = input
	.map(x => {
		if(x.substring(0,2) === "me") {
			return {address: +x.split("[")[1].split("]")[0], val: +x.split(" = ")[1]};
		} else {
			const maskStr = x.substring("mask = ".length);
			return {mask: [...maskStr].map(x => +x).reverse()}
		}
	});

const applyMask = (val, mask) => {
	for(var i = 0n; i < mask.length; i++) {
...
const printMask = (mask) => mask.map(x => Number.isNaN(x) ? "X" : ""+x).join("")
...
instructions.forEach(ins => {
	if(ins.mask) {
		...
		mem[ins.address] = applyMask(BigInt(ins.val), mask);
	}
});
...
const printTree = (node, prefix) => {
	for(let entry of Object.entries(node)) {
		if("value" in entry[1]) {
			console.log(prefix + "[" + entry[0] + "] = " + entry[1].value);
		} else {
			console.log(prefix + "[" + entry[0] + "]");
			printTree(entry[1], prefix + "|  ");
		}
	}
}
...
instructions.forEach(ins => {
	if(ins.mask) {
		...
		const set = (node, i) => {
			...
			const bitMask = 1n << i;
			let bit = bitMask & BigInt(ins.address);
			if(Number.isNaN(mask[i])) {
				bit = "X";
			} else if(mask[i] == 0) {
				bit = (bit === 0n ? "0" : "1");
			...
			// There's already an X, but we aren't X
			if("X" in node && bit !== "X") {
				// Clone X and go down one route.
				node["1"] = node["X"];
				node["0"] = JSON.parse(JSON.stringify(node["X"]));
				delete node["X"];
			}
			// There's already a 0 or 1, but we are X
			if(("1" in node || "0" in node) && bit === "X") {
				node["1"] = node["1"] || {};
				node["0"] = node["0"] || {};
				return [
					() => set(node["1"], i - 1n),
			...
			return [() => set(node[bit], --i)];
		}
		
		let q = [() => set(tree, BigInt(35))];
		while(q.length) {
			const f = q.pop();
			const res = f();
			if(res) {
...
const sumTree = (node, mult) => {
	if(!node) {
		return 0;
	}
	if("value" in node) {
		return node.value * mult;
	}
	
	if("X" in node) {
		return sumTree(node["X"], mult * 2);
	}
	
	return sumTree(node["0"], mult) + sumTree(node["1"], mult);
}

console.log(sumTree(tree, 1));
//printTree(tree, "");
